...
async function resizeImage(imageName:string,hight:number,width:number){
    ...
    const outputPath:string=path.resolve(`assets/images/resize/${imageName}_${hight}_${width}.jpg`);
    await sharp(inputPath).resize(width,hight).toFile(outputPath).then(():string=>{
